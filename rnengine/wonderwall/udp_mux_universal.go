...
func (m *UniversalUDPMuxDefault) ReadFromConn(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			m.log.Logger.Debugf("stopped reading from the UDPConn due to finished context")
			return
		default:
			n, a, err := m.params.UDPConn.ReadFrom(buf)
			if err != nil {
				m.log.Logger.Errorf("error while reading packet: %s", err)
				continue
			}
			msg := &stun.Message{
			...
			err = msg.Decode()
			if err != nil {
				m.log.Logger.Warnf("error while parsing STUN message. The packet doesn't seem to be a STUN packet: %s", err)
				continue
			}

			err = m.HandleSTUNMessage(msg, a)
			if err != nil {
				m.log.Logger.Errorf("error while handling STUn message: %s", err)
			}
		}
	}
}
...
func (m *UniversalUDPMuxDefault) HandleSTUNMessage(msg *stun.Message, addr net.Addr) error {
	...
	if m.isXORMappedResponse(msg, udpAddr.String()) {
		...
		if err != nil {
			m.log.Logger.Debugf("%s: %v", fmt.Errorf("failed to get XOR-MAPPED-ADDRESS response"), err)
			return nil
		}
		return nil
	}
