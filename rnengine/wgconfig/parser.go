...
func FromUAPI(r io.Reader) (*WgConfig, error) {
	cfg := new(WgConfig)
...
func (cfg *WgConfig) handleDeviceLine(k, value mem.RO) error {
	switch {
...
func (cfg *WgConfig) handlePublicKeyLine(valueBytes []byte) (*Peer, error) {
	p := Peer{}
	var err error
...
// endpointにはiceから取得したremote ipがセットされる
func (cfg *WgConfig) handlePeerLine(peer *Peer, k, value mem.RO, valueBytes []byte) error {
	switch {
	case k.EqualString("endpoint"):
		netudp := net.UDPAddr{}
		err := netudp.IP.UnmarshalText(valueBytes)
		if err != nil {
			return fmt.Errorf("handle peer line error, can not parsed endpoint")
		}
		peer.Endpoint = &netudp
